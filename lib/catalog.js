...
import { SITE, shapePodcast, shapeEpisode, podcastImage } from "@/lib/catalog-shared";

export { SITE, TESTAMENTS, podcastImage } from "@/lib/catalog-shared";

// Admin-editable site config (branding + pricing) from the site_settings row.
// Falls back to the SITE constants for any field until the row exists, or if
// the query fails (e.g. a CI build without Supabase env). Wrapped in React
// cache() so the layout, metadata, and pages share one query per request.
export const getSettings = cache(async () => {
  try {
    ...
    return {
      ...
      email:        data?.support_email ?? SITE.email,
      price:        data?.episode_price != null ? Number(data.episode_price) : SITE.price,
      primaryColor: data?.primary_color ?? "#660e1b",
    };
  } catch {
    return { name: SITE.name, email: SITE.email, price: SITE.price, primaryColor: "#660e1b" };
  }
});
