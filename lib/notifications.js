...
// Owner inbox that receives the internal "tracking" copies (new sign-up,
// new sale). Set NEXT_PUBLIC_ADMIN_EMAIL in .env.local to your real inbox.
const ADMIN_INBOX =
...
export function notifySignup(user) {
  ...
  // Welcome email to the new member. (Supabase Auth sends the real
  // "Confirm your email" message separately — we don't duplicate it here.)
  sendEmail({
    to: user.email,
    template: "welcome",
    data: { name: user.name },
...
// ─── Commerce ──────────────────────────────────────────────────────
// Sends a single "new sale" tracking email to the owner (ADMIN_INBOX).
// We deliberately do NOT email the customer — that would require a Resend-
// verified sending domain. The owner copy goes to your own inbox, which
// Resend allows from onboarding@resend.dev with no domain setup.
